...
namespace SDRSharp.Radio.PortAudio
{
	public class WaveDuplex : IDisposable
	{
		...
		public unsafe WaveDuplex(int deviceIndex, double sampleRate, int framesPerBuffer, AudioBufferAvailableDelegate bufferNeededDelegate)
		{
			...
			if (paError != 0)
			{
				throw new ApplicationException("Init audio in/out device: " + paError.ToString());
			}
			...
			{
				this._gcHandle.Free();
				throw new ApplicationException("Open audio in/out device: " + paError.ToString());
			}
			...
			this._gcHandle.Free();
			throw new ApplicationException("Start audio in/out device: " + paError.ToString());
		}
