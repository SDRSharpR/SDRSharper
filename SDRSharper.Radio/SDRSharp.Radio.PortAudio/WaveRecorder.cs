...
namespace SDRSharp.Radio.PortAudio
{
	public class WaveRecorder : IDisposable
	{
		...
		public unsafe WaveRecorder(int deviceIndex, double sampleRate, int framesPerBuffer, AudioBufferAvailableDelegate bufferAvailable)
		{
			...
			if (paError != 0)
			{
				throw new ApplicationException("Init audio input device: " + paError.ToString());
			}
			...
			{
				this._gcHandle.Free();
				throw new ApplicationException("Open audio input device: " + paError.ToString());
			}
			...
			this._gcHandle.Free();
			throw new ApplicationException("Start audio input device: " + paError.ToString());
		}
