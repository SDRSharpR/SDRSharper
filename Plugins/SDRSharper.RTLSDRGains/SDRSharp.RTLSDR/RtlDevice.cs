...
namespace SDRSharp.RTLSDR
{
	public sealed class RtlDevice : IDisposable
	{
		...
		private unsafe static readonly float* _lutPtr;

		private static readonly UnsafeBuffer _lutBuffer;
		...
		private int _vgaGain;

		private int _lnaGain;

		private int _mixerGain;
		...
		private static readonly RtlSdrReadAsyncDelegate _rtlCallback;

		private static readonly uint _readLength;
		...
		public int Gain
		{
			...
			set
			{
				...
				if (this._dev != IntPtr.Zero)
				{
					NativeMethods.rtlsdr_set_tuner_gain_ext(this._dev, this._lnaGain, this._mixerGain, this._vgaGain);
				}
			}
		}
		...
		public void Start()
		{
			...
			if (!this._useTunerAGC && NativeMethods.rtlsdr_set_tuner_gain(this._dev, this._vgaGain) != 0)
			{
				throw new ApplicationException("Cannot access RTL device");
			}
			if (NativeMethods.rtlsdr_set_tuner_gain_ext(this._dev, this._lnaGain, this._mixerGain, this._vgaGain) != 0)
			{
				throw new ApplicationException("Cannot set extended gain values");
			}
		...
		private unsafe static void RtlSdrSamplesAvailable(byte* buf, uint len, IntPtr ctx)
		{
			...
			if (gCHandle.IsAllocated)
			{
				...
				{
					Complex* intPtr = ptr;
					float* lutPtr = RtlDevice._lutPtr;
					byte* intPtr2 = buf;
					buf = intPtr2 + 1;
					intPtr->Imag = lutPtr[(int)(*intPtr2)];
					Complex* intPtr3 = ptr;
					float* lutPtr2 = RtlDevice._lutPtr;
					byte* intPtr4 = buf;
					buf = intPtr4 + 1;
					intPtr3->Real = lutPtr2[(int)(*intPtr4)];
					ptr++;
				}
